...
export const ReportConnectedPacUserFlavorList: FC<ReportProps> = ({
  ...
  const onSubmit = async (queryRequest: ReportService.QueryRequest) => {
    await logClick("ReportConnectedPacUserFlavorList","search","");
    setQuery({ ...queryRequest });
  };

  const onPageSelection = async (pageNumber: number) => {
    await logClick("ReportConnectedPacUserFlavorList","selectPage",pageNumber.toString());
    setQuery({ ...query, pageNumber: pageNumber });
  };

  const onPageSizeChange = async (pageSize: number) => {
    await logClick("ReportConnectedPacUserFlavorList","pageSizeChange",pageSize.toString());
  ...
  const onRefreshRequest = async () => {
    await logClick("ReportConnectedPacUserFlavorList","refresh","");
    setQuery({ ...query });
  };

  const onSort = async (columnName: string) => {
    await logClick("ReportConnectedPacUserFlavorList","sort",columnName);
  ...
  const onExport = async () => {
    await logClick("ReportConnectedPacUserFlavorList","export","");
    if(isProcessing){
      return;
    }
